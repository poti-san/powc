...
class BindHandlerID:
    SFOBJECT = GUID("{3981e224-f559-11d3-8e3a-00c04f6837d5}")
    SFUIOBJECT = GUID("{3981e225-f559-11d3-8e3a-00c04f6837d5}")
    SFVIEWOBJECT = GUID("{3981e226-f559-11d3-8e3a-00c04f6837d5}")
    STORAGE = GUID("{3981e227-f559-11d3-8e3a-00c04f6837d5}")
    STREAM = GUID("{1CEBB3AB-7C10-499a-A417-92CA16C4CB83}")
    RANDOMACCESSSTREAM = GUID("{f16fc93b-77ae-4cfe-bda7-a866eea6878d}")
    LINKTARGETITEM = GUID("{3981e228-f559-11d3-8e3a-00c04f6837d5}")
    STORAGEENUM = GUID("{4621A4E3-F0D6-4773-8A9C-46E77B174840}")
    TRANSFER = GUID("{5D080304-FE2C-48fc-84CE-CF620B0F3C53}")
    PROPERTYSTORE = GUID("{0384e1a4-1523-439c-a4c8-ab911052f586}")
    THUMBNAILHANDLER = GUID("{7b2e650a-8e20-4f4a-b09e-6597afc72fb0}")
    ENUMITEMS = GUID("{94f60519-2850-4924-aa5a-d15e84868039}")
    DATAOBJECT = GUID("{B8C0BD9F-ED24-455c-83E6-D5390C4FE8C4}")
    ASSOCARRAY = GUID("{bea9ef17-82f1-4f60-9284-4f8db75c3be9}")
    FILTER = GUID("{38d08778-f557-4690-9ebf-ba54706ad8f7}")
    ENUMASSOCHANDLERS = GUID("{b8ab0b9c-c2ec-4f7a-918d-314900e6280a}")
    STORAGEITEM = GUID("{404e2109-77d2-4699-a5a0-4fdf10db9837}")
    FILEPLACEHOLDER = GUID("{8677DCEB-AAE0-4005-8D3D-547FA852F825}")
